...
- **Verified on device:** castle-guard size→2 wrote `scale = 2` (the `addField` insert branch, since it had no scale) into `castle-guard-e17735.character`; `resistance`/weapon/`name` preserved; the enemy visibly enlarged in-canvas; button only shows on `character_spawn` (not on `mobs` spawn-pools — those are shared random pools, out of scope).
- **Deploy reality:** to play it, export BOTH the level and the custom `.character` (drop into `npcs/<category>/`) — same as the level editor already.
- Code: `LevelViewerActivity` (`btnEditEnemy` wiring, `isCharacterEntity`/`charRefKey`/`loadCharacterDoc`/`findWeaponBlock`/`showEnemyEditDialog`/`applyEnemyEdit`), `activity_level_viewer.xml` (`btnEditEnemy` in the inspector), `CharacterDocument.setOrAdd`, `strings.xml`.
- **Automation gotcha (cost me time):** the soft keyboard covers the popup's SAVE — `adb shell input keyevent 4` to dismiss it BEFORE tapping SAVE.
**Last updated:** 2026-06-18 (inline-spawner palette added — Phase 8 fully complete)
**Last QA pass:** 2026-06-20 — full matrix re-verified on HEAD (emulator `Pixel_10_Pro_XL`, fresh install). Logcat clean throughout. Save fidelity by semantic XML diff vs source: no-op = **0 diffs** (589 ents); inspector Z-edit, duplicate, delete, palette-add, CustomData (value-edit + add-field), and ★spawner-template each diffed to **exactly** their own change (spawner block round-trips with `<Scale>=4` intact). Also passed: undo/redo across types, move + grid-snap on/off, on-canvas rotate(→−80°)/scale handles, marquee group-delete (−15) + undo, entity-browser search+center, validation warn on missing `spawn0`, palette thumbnails, tap-to-select / deselect-on-empty / z-cycle, SAF export to Downloads (0-diff), My Levels reopen + re-save-from-storage (0-diff). No regressions found.
...
- [x] **CustomData editing (add / remove / edit + undo)** in the inspector — tap the meta line (✎) → edit existing values, **remove** fields (✕), and **add** fields (name + engine type `string/int/uint/float` + value). Persisted via a `customDataEdited`-gated `patchNode` that diffs the in-memory map against the DOM: update existing `<Value>`s, delete removed `<Variable>`s, append added ones. **Inserts are gated on `customDataTypes`** (only keys the user explicitly added) so a FileName-referenced entity isn't ballooned with its `.ent`-resolved keys. The whole edit is one undoable command. Verified on emulator via semantic diff on a survivor inline entity (`gold_loot`): add `testKey=(int,42)` + remove `premiumEffect` + value update → exactly those changes, **0 diffs across the other 588 entities**, no-op still 0-diff, undo restores the original. Touches: `LevelEntity.java` (`customDataTypes`), `LevelSaver.java` (`patchNode` CustomData diff), `LevelViewerActivity.java` (`showCustomDataDialog`/`addCustomDataRow`/`showAddFieldDialog` + undo command), `strings.xml`.
- [x] **Inline-defined entity (spawner) palette** — inline-defined entities (spawners, buttons, invisible collision...) that have no `.ent` to FileName-reference can now be placed from harvested **real blocks**. A few real blocks are bundled under `assets/spawner_templates/` (`duel_mob_spawn`, `button`, `invisible_one_sided_platform`) and merged into the Add palette (marked `★`, branched in the click handler). Adding parses the block for rendering (via a minimal `<Ethanon><EntitiesInScene>…` envelope through `LevelParser`) and carries the raw block on the entity (`sourceBlockXml`). `LevelSaver` gained a **gated** `buildNewNode` branch that writes that block **verbatim** (only id + Position/flip swapped) — gated on `sourceBlockXml != null`, so it's inert for every pre-existing flow. Because `scaleEdited` stays false, the NPC `<Scale>` survives (the old 4→1 trap). **Verified on emulator (semantic diff):** no-op save = 0 diffs (589 ents); add `duel_mob_spawn` → +1 entity, 0 diffs on the originals, and the saved block is semantically identical to the template modulo id+position with `<Scale>=4` intact. Touches: `LevelEntity.java` (`sourceBlockXml`), `LevelSaver.java` (`buildNewNode` branch + `parseBlock`), `LevelRenderView.java` (`addSpawnerTemplate`), `LevelViewerActivity.java` (template load + palette merge), `assets/spawner_templates/`.
- Note: bundled template set is a starter subset (3 real blocks); add more by dropping harvested `<Entity>` blocks into `assets/spawner_templates/`. Template rows show no thumbnail (no `.ent` to resolve); invisible entities render as just a selection box (correct).
- [x] **Multi-select (marquee + group move/delete)** — an edit-mode marquee toggle (`btnMarquee`); with it on, dragging a box selects every entity whose **rendered** center (`computeRenderY`, not raw `y`) is inside. A group moves together (grid-snapped, relative layout preserved) and deletes as one undoable command; the Delete button enables for groups, Duplicate stays single-only. **Inert by design:** marquee off + selection ≤1 leaves every single-select/move/pan/handle path byte-identical (new branches gated on `marqueeMode` / `multiSelection.size()>1`); single-select clears the group to hold the invariant. Verified on emulator: marquee picked 21 entities; **group-delete + undo** restored all 21; **group-move + undo** restored exactly; single-select / handles / move unaffected. Touches: `LevelRenderView.java` (touch branches, marquee + group ops, `GroupMove/GroupDeleteCommand`, `drawHandles` param), `LevelViewerActivity.java` (marquee button, group delete, multi-select toast), `activity_level_viewer.xml`, `strings.xml`.
- [x] **Entity browser (find & jump)** — an edit-mode top-bar button (`btnEntityList`) opens a searchable list of *every* entity in the level (`name · id · @x,y`); tapping one **selects it and pans the view to center it**. Solves the real pain that tapping to select is unreliable when an entity has no sprite (e.g. `spawn0`) or a stored center offset from its sprite — and dense levels have ~589 entities. Search matches name or id (`ArrayAdapter` filter over an `EntityRef.toString()`). Verified on emulator: searched `spawn0` → tapped → view centered on it, selected, inspector shows its real position. Touches: `LevelRenderView.java` (`getEntities`/`selectAndCenter`), `LevelViewerActivity.java` (`showEntityBrowser` + `EntityRef`), `activity_level_viewer.xml`, `strings.xml`. (Beyond the original Phase 8 list — added on request to "go crazy".)
...
- Note: CustomData add/remove/edit is now undoable (one command per dialog Save). Removing a `.ent`-resolved key on a FileName-referenced entity is a no-op on save (the key lives in the `.ent`, not the inline DOM) — only inline keys and user-added overrides are written. Soft keyboard can overlap the dialog's buttons — scroll if needed.
