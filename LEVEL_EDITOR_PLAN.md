...
**Status:** Phases 0–8 COMPLETE + **ALL** Phase 8 extras shipped (validation warnings · palette thumbnails · on-canvas rotate/scale handles · inline-spawner palette · **entity browser**). Full editor verified on emulator: find-entity(search+center) / select / inspect / move(+snap) / rotate+scale on canvas / add(+sprite-preview palette, +inline-spawner templates) / duplicate / delete / undo-redo, Save→"My Levels"→reopen, Export (SAF) to Downloads, CustomData value editing, and validation. Saves are surgical (no-op = 0 diffs; each edit = exactly its change; inline-spawner blocks round-trip faithfully incl. NPC `<Scale>`).
